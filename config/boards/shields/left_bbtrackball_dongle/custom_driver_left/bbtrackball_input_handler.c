/*
 * bbtrackball_input_handler.c - Dongle fusion version
 * Event-driven architecture from ZitaoTech, pos 61 only, no CapsLock.
...
static int bbtrackball_init(const struct device *dev) {
    ...
    LOG_INF("Initializing BBtrackball (event-driven)");

    data->dev = dev;

    k_work_queue_start(&bbtrackball_work_q, bbtrackball_workq_stack,
                       K_THREAD_STACK_SIZEOF(bbtrackball_workq_stack),
                       BBTRACKBALL_WORKQ_PRIORITY, NULL);
