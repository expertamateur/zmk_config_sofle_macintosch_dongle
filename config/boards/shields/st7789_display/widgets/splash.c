...
static const uint32_t rainbow_green = 0x73B55B;
static const uint32_t rainbow_yellow = 0xF3C34D;
static const uint32_t rainbow_orange = 0xE98235;
static const uint32_t rainbow_red = 0xD94A4B;
static const uint32_t rainbow_purple = 0x8E428F;
static const uint32_t rainbow_blue = 0x4798C8;

static uint16_t rainbow_color_for_y(uint16_t y) {
    if (y < 40) {
        return rgb888_to_rgb565(rainbow_green);
    }
    if (y < 80) {
        return rgb888_to_rgb565(rainbow_yellow);
    }
    if (y < 120) {
        return rgb888_to_rgb565(rainbow_orange);
    }
    if (y < 160) {
        return rgb888_to_rgb565(rainbow_red);
    }
    if (y < 200) {
        return rgb888_to_rgb565(rainbow_purple);
    }
    return rgb888_to_rgb565(rainbow_blue);
}

static void print_rainbow_background(void) {
    /* Six equal-height bands fill the complete 240 x 240 panel. */
    print_filled_screen_area(0, 0, 240, 40, rgb888_to_rgb565(rainbow_green));
    print_filled_screen_area(0, 40, 240, 40, rgb888_to_rgb565(rainbow_yellow));
    print_filled_screen_area(0, 80, 240, 40, rgb888_to_rgb565(rainbow_orange));
    print_filled_screen_area(0, 120, 240, 40, rgb888_to_rgb565(rainbow_red));
    print_filled_screen_area(0, 160, 240, 40, rgb888_to_rgb565(rainbow_purple));
    print_filled_screen_area(0, 200, 240, 40, rgb888_to_rgb565(rainbow_blue));
}

void print_snake_image() {
    for (uint16_t i = 0; i < snake_image_height; i++) {
        uint16_t screen_y = snake_image_y + i;
        render_bitmap(snake_image_buf, snake_image[i], snake_image_x, snake_image_y + i,
                      snake_image_width, 1, 1, get_splash_logo_color(),
                      rainbow_color_for_y(screen_y));
    }
}
...
void zmk_widget_splash_init() { buffer_snake_image_init(); }

void clean_up_splash() { k_free(snake_image_buf); }
...
void invalidate_splash(void) { splash_drawn = false; }
